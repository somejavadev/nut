...
// consider WITH_DMF_FUNCTIONS too?
	if (fun || nuf) {
		upslogx(1, "WARNING : DMF does not support lookup functions at this time, so the provided value is effectively ignored: fun='%s' nuf='%s'", fun, nuf);
	}
	self->fun = NULL;
	self->nuf = NULL;
#endif // WITH_SNMP_LKP_FUN
